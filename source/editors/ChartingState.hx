...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['coolcolormodeoff', "Value 1: Character to change (Dad, BF, GF)\nValue 2: New character's name"],
		['newlyrics', "Value 1: time till lyrics fades \nValue 2: lyrics"],
	...
	**/
	...
	function loadSong():Void
	{
		...
		var filebf:Dynamic = Paths.voicesbf(currentSongName);
		vocals = new FlxSound();
		if (Std.isOfType(filebf, Sound) || OpenFlAssets.exists(filebf)) {
			vocals.loadEmbedded(filebf);
			FlxG.sound.list.add(vocals);
		}

		var filedad:Dynamic = Paths.voicesdad(currentSongName);
		oppvocals = new FlxSound();
		if (Std.isOfType(filedad, Sound) || OpenFlAssets.exists(filedad)) {
			oppvocals.loadEmbedded(filedad);
