...
//lyrics shit
	...
	//genisis stage (spotlite)
	public static var overlay:BGSprite;
	
	public static var eventtrigger:String;
	public var sbg:BGSprite;
	...
	public var shadervar:Float =0.5;
	...
	var testshadermec:Bool = false;
	var timermod:Float = 4.0;
	public var hittime:Bool;
	...
	override public function create()
	{
		...
		curStage = SONG.stage;
		switch (curStage){  //precash items. or set up stage specific things. like stage shaders
			case ('spotlite'):
				precacheList.set('stages/base', 'image');
				precacheList.set('stages/overlay', 'image');
		...
		switch (curStage)
		{
			...
			case 'spotlite':
				...
				sbg = new BGSprite('stages/base', -200, -100, 0.9, 0.9);
				sbg.shader = wiggle.shader;
				sbg.setGraphicSize(Std.int(sbg.width * 2));
				add(sbg);
				overlay = new BGSprite('stages/overlay', -200, -100, 0.9, 0.9);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsoppVoices)
			oppvocals = new FlxSound().loadEmbedded(Paths.voicesdad(PlayState.SONG.song));
		else
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (!isStoryMode && !skipArrowStartTween)
			{
				//babyArrow.y -= 10;
				babyArrow.alpha = 0;
				if (dad.curCharacter != 'genisis' && player == 0 || player == 1) //this is a hack and a half im sorry reader
			...
			{
				if (dad.curCharacter != 'genisis' && player == 0 )
					babyArrow.alpha = 0;
				else if (dad.curCharacter != 'genisis' && player == 1)
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'stage':
				sheddar.setFloat("time", Sys.cpuTime() * elapsed);
				sheddar.setFloat("amount", shadervar);
		...
		if (testshadermec == true && canPause){ //bassed of hypno luliby code cause this math is to big for my brain. modified for use here
			var timemod:Float =((Conductor.songPosition/ Conductor.crochet * 2)/ Math.PI);
			var timeframe = Math.floor(((timemod/Math.PI)-Math.floor(timemod/Math.PI))*1000)/1000;
			var doit:Float = 0.2;
			
			if (timeframe < doit || timeframe > (1-doit))
				{
					if (!hit){
						hittime = true;
						trace('hitnow');
			...
			else if (FlxG.keys.justPressed.SPACE && hittime ){
					
					hit = true;
					if(hittime){
						if (camOther.alpha != 0.0){
							camOther.alpha -=0.1;
							warning.alpha = 0;
							trace('good');
							hittime = false;
		...
		if (Paths.formatToSongPath(SONG.song) == 'lovely-sound' ){
			switch (curBeat){
				...
				case(46):
				moveCamera(true);
				FlxTween.tween(dad,{alpha: 1.0}, 0.9, {ease: FlxEase.cubeInOut});
				FlxTween.tween(iconP2,{alpha: 1.0}, 0.9, {ease: FlxEase.cubeInOut});
				FlxTween.tween(FlxG.camera, {zoom: 0.65}, 1.9,  {
					ease: FlxEase.linear,
					onComplete: function(twn:FlxTween)
						{
							healthBar.leftBar.color = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
							defaultCamZoom =0.65;
						}});
				case(303):
					boyfriendCameraOffset[0] = 0;
				...
				case(319):
					cameraTwn = FlxTween.tween(FlxG.camera, {zoom: 0.65}, (1.0), {ease: FlxEase.elasticInOut, onComplete:
		...
		if (FlxG.keys.anyJustPressed(debugKeysChart) && !endingSong && !inCutscene)
		{
		 ...
		 FlxG.watch.addQuick('shader', camOther.alpha);
			...
			if(startedCountdown)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					...
					switch (daNote.noteType){
						...
						case ('woodhog test'):
							if (health >= 1 &&daNote.texture != 'HURTNOTE_assets' ){
								
								daNote.texture = 'HURTNOTE_assets';
							}
							if (health < 1 && daNote.texture == 'HURTNOTE_assets'){
								
	
								daNote.texture = 'NOTE_assets';
							}

						case('speed test'):
							ModchartFuncs.startMod('speed', 'speedModifier', 'lane', -9090);
					}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'newlyrics':
				makelyrics(value2, Std.parseFloat(value1));

			case 'coolcolormodeon':
			
				
			trace(boyfriend.color);
			switch(curStage){
		...
		case 'coolcolormodeoff':
			...
			switch(curStage){
				...
				case ('spotlite'):
                    ...
                    FlxTween.color(dad, 1.5, FlxColor.BLACK, FlxColor.WHITE, {ease: FlxEase.quadInOut});
					overlay.shader = wiggle.shader;
	...
	function moveCameraSection():Void {
		...
		{
			if (Paths.formatToSongPath(SONG.song) == 'lovely-sound' && curBeat >=207 && curBeat<= 240 ||  Paths.formatToSongPath(SONG.song) == 'lovely-sound' && curBeat >= 319 ){
				camFollow.set(gf.getMidpoint().x, gf.getMidpoint().y);
				camFollow.x += gf.cameraPosition[0] + girlfriendCameraOffset[0];
				camFollow.y += gf.cameraPosition[1] + girlfriendCameraOffset[1];  //this is a hack for centered screen
			}
			else{
				moveCamera(true);
			}
	...
	function opponentNoteHit(note:Note):Void
		{
			...
			else if(!note.noAnimation && note.noteType != 'No Animation') {
		...
		function goodNoteHit(note:Note):Void
		{
			...
			if (!note.wasGoodHit)
			{
				...
				if(!note.noAnimation && note.noteType != 'No Animation') {
	...
	public function makelyrics(lyric:String, length:Float){ //modified from sonic rodentrap. go play it
		lyrics.alpha = 1;
		lyrics.text = lyric;
